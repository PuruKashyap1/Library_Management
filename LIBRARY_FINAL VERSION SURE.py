...
# Create table with the correct order of columns
connector.execute(
    'CREATE TABLE IF NOT EXISTS Library (BK_ID TEXT PRIMARY KEY NOT NULL, BK_NAME TEXT, AUTHOR_NAME TEXT, BK_STATUS TEXT, CARD_ID TEXT)'
)
...
def display_records():
    ...
    curr = connector.execute('SELECT BK_ID, BK_NAME, AUTHOR_NAME, BK_STATUS, CARD_ID FROM Library')
    data = curr.fetchall()

    for idx, records in enumerate(data):
        tree.insert('', END, values=(records[0], records[1], records[2], records[3], records[4]), tags=(str(idx % 2),))
...
def add_record():
    ...
    if surety:
        try:
            connector.execute(
                'INSERT INTO Library (BK_ID, BK_NAME, AUTHOR_NAME, BK_STATUS, CARD_ID) VALUES (?, ?, ?, ?, ?)',
                (bk_id.get(), bk_name.get(), author_name.get(), bk_status.get(), card_id.get()))
            connector.commit()
...
# Left Frame
Label(left_frame, text='Book ID', bg=lf_bg, font=lbl_font).place(relx=0.5, rely=0.05, anchor=CENTER)
bk_id_entry = Entry(left_frame, width=25, font=entry_font, textvariable=bk_id)
bk_id_entry.place(relx=0.5, rely=0.1, anchor=CENTER)

Label(left_frame, text='Book Name', bg=lf_bg, font=lbl_font).place(relx=0.5, rely=0.2, anchor=CENTER)
Entry(left_frame, width=25, font=entry_font, textvariable=bk_name).place(relx=0.5, rely=0.25, anchor=CENTER)

Label(left_frame, text='Author Name', bg=lf_bg, font=lbl_font).place(relx=0.5, rely=0.35, anchor=CENTER)
Entry(left_frame, width=25, font=entry_font, textvariable=author_name).place(relx=0.5, rely=0.4, anchor=CENTER)
...
tree = ttk.Treeview(RB_frame, selectmode=BROWSE, columns=('Book ID', 'Book Name', 'Author', 'Status', 'Issuer Card ID'))
...
tree.column('#2', width=200, stretch=YES, anchor=CENTER)
tree.column('#3', width=100, stretch=YES, anchor=CENTER)
